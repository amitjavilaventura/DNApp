        ...
        splitLayout(cellWidths = c("25%", "75%"),
          column(width = 12,
                 box(width = 12, collapsible = F, collapsed = F,
                     ...
                     textAreaInput(inputId = "input_seq", label = "Write the query sequence", height = 150),
                     actionButton(inputId = "submit_input", label = "Submit")),
                 box(width = 12, collapsible = T, collapsed = T,
                     title = tagList(icon("gear"), "General params"), solidHeader = T,
                     selectInput(inputId = "nucleic_acid", label = "Select the nucleic acid:", choices = c("DNA", "RNA"), selected = "DNA", selectize = T),
                     sliderInput(inputId = "start", label = "Base to start reading (1,2,3)", value = 1, min = 1, max = 3, step = 1)),
                 box(title = tagList(icon("chart-bar"), "Stats params"), width = 12, collapsible = T, collapsed = T,
                     actionButton(inputId = "show_stats", label = "Show stats")),
                 box(title = tagList(icon("sort-alpha-down"), "Conversion parms"), width = 12, collapsible = T, collapsed = T,
                     actionButton(inputId = "do_conversion", label = "Do conversion")),
                 #box(title = tagList(icon("cut"), "Restriction params"), width = 12, collapsible = T, collapsed = T,
                 #    actionButton(inputId = "do_restriction", label = "Restriction targets")),
                 box(title = tagList(icon("dna"), "DNA2prot params"), width = 12, collapsible = T, collapsed = T,
                     actionButton(inputId = "do_dna2prot", label = "Translate DNA"))),
           tabsetPanel(id = "snap", type = "pills",
              tabPanel(title = "Statistics", value = "stats"),
              tabPanel(title = "Seq. conversion", value = "transform"),
              #tabPanel(title = "Restriction targets", value = "restriction"),
              tabPanel(title = "DNA2Prot", value = "2prot"))))
