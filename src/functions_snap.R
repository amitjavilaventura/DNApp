...
# DNA FUNCTIONS -----------------------------------------------------------------------------------
is.dna <- function(input){
  
  library(dplyr)
  library(stringr)
  
  splited <- stringr::str_split(string = input, pattern = "") %>% unlist()
  check.dna <- grepl("[ATGCatgc]", splited)
...
dna.stats <- function(input, start = 1){
  ...
  if(!is.dna(input)){ return("The input DNA must contain only A,T,G and C.")}
  else{
...
dna.conversion <- function(input, start = 1){
...
dna2prot <- function(input, start = 1, code = "standard"){
...
# RNA FUNCTIONS -----------------------------------------------------------------------------------
is.rna <- function(input){
  
  library(dplyr)
  library(stringr)
  
  splited <- stringr::str_split(string = input, pattern = "") %>% unlist()
  check.rna <- grepl("[AUGCaugc]", splited)
  
  if(FALSE %in% check.rna){ return(FALSE) }
  else{ return(TRUE) }
}

rna.stats <- function(input, start = 1){
  ...
  if(!is.rna(input)){ return("The input RNA must contain only A,U,G and C.") }
  else{
    ...
    out <- tibble(Stat  = c("Input", "Length", "% GC", "Number of codons"),
                  Value = c(input, len, gc, n_codons))
